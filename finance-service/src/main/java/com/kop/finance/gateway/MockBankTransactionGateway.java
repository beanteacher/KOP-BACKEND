...
/**
 * {@link BankTransactionGateway} 기본 구현체 — 항상 빈 목록을 반환한다. 실제 은행 데이터를
 ...
 * openbanking.gateway-provider가 없거나 "mock"이면 활성화된다(matchIfMissing=true) — 실제
 * API 호출은 {@link RealBankTransactionGateway}(gateway-provider=real)가 담당한다.
 */
@Component
@ConditionalOnProperty(name = "openbanking.gateway-provider", havingValue = "mock", matchIfMissing = true)
